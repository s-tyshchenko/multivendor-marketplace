@if (Cart::instance('cart')->count() > 0)
        ...
        @foreach(Cart::instance('cart')->content() as $key => $cartItem)
            @php
                ...
                if ($cartItem->isCustom) {
                    $product = new \Botble\Ecommerce\Models\Custom\Product($cartItem->toArray());
                }
            @endphp
                ...
                <li>
                    <div class="shopping-cart-img">
                        <a href="{{ $product->original_product->url }}"><img
                                ...
                                src="{{ RvMedia::getImageUrl($cartItem->options['image'], 'thumb', false, RvMedia::getDefaultImage()) }}"></a>
                    </div>
                    <div class="shopping-cart-title">
                        <h4>
                            <a href="{{ $product->original_product->url }}">{{ $product->original_product->name }}  @if ($product->isOutOfStock())
                                    <span class="stock-status-label">({!! $product->stock_status_html !!})</span> @endif
                            </a></h4>
                        <h3><span class="d-inline-block">{{ $cartItem->qty }}</span> <span
                                class="d-inline-block"> x </span> <span
                                class="d-inline-block">{{ format_price($cartItem->price) }}</span> @if ($product->front_sale_price != $product->price)
                                <small>
                    ...
                    <div class="shopping-cart-delete">
                        <a href="#" data-url="{{ route('public.ajax.cart.destroy', $cartItem->rowId) }}"
                           class="remove-cart-item"><i class="fi-rs-cross-small"></i></a>
                    </div>
                </li>
            @endif
        @endforeach
    ...
    <div class="shopping-cart-footer">
        <div class="shopping-cart-total">
            ...
            @else
                <h4><strong class="d-inline-block">{{ __('Sub Total') }}:</strong>
                    <span>{{ format_price(Cart::instance('cart')->rawSubTotal()) }}</span></h4>
            @endif
        </div>
